...
$conn->query(
    "UPDATE orders o
     ...
     WHERE o.id=$order_id
       ...
       AND LOWER(TRIM(o.status)) IN ('confirmed','preparing')"
);

if ($conn->affected_rows > 0) {
    $msg = "Order is ready for delivery";
    $conn->query(
        "INSERT INTO order_tracking (order_id, status, message)
         VALUES ($order_id, 'Ready', '$msg')"
    );
}

echo json_encode([
    "success" => $conn->affected_rows > 0,
    "message" => $conn->affected_rows > 0 ? "Order marked as ready" : "Unable to update order"
]);
